...
## 🎨 החלפת לוגו וכותרת בעתיד

### החלפת לוגו
1. הכינו קובץ PNG חדש (מומלץ רקע שקוף, 200×200 פיקסל לפחות)
2. שמרו אותו כ-`public/logo.png` (החליפו את הקובץ הקיים)
3. הלוגו יתעדכן אוטומטית בכל האתר (Header + דף הבית)

הקומפוננטה שמנהלת את הלוגו: `src/components/ui/Logo.tsx`

### החלפת כותרת האתר
1. פתחו `src/components/ui/Header.tsx`
2. שנו את השורה: `const SITE_TITLE = "..."`
3. פתחו `src/app/layout.tsx` ושנו את `title` ו-`description` ב-metadata
4. פתחו `src/app/page.tsx` ושנו את הכותרת ב-`<h1>`

**מומלץ**: חפשו את הטקסט הנוכחי בכל הפרויקט עם: `grep -r "שם הכותרת" src/`
